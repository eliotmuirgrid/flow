...
int luaL_getn (lua_State *L, int t) {
  COL_FUNCTION(luaL_getn);
  int n;
  COL_VAR(t);
  t = abs_index(L, t);
  COL_VAR(t);
  lua_pushliteral(L, "n");  /* try t.n */
  COL_TRC("Pushed literal n");
  COLASSERT(t <= lua_gettop(L));
  COLASSERT(lua_istable(L, t));
  lua_rawget(L, t);
  COL_TRC("After rawget");
  if ((n = checkint(L, 1)) >= 0){
    COL_VAR(n);
    return n;
  }   
...
// TODO circular require shouldn't kill things.
LUALIB_API int luaL_loadfile (lua_State *L, const char *filename) {
  COL_FUNCTION(luaL_loadfile);
  LoadF lf;
